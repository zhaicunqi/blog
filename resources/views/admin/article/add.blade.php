...
@section('content')
	<form action="{{url('/admin/article/add')}}" id="dataForm">
		<div class="row">
						...
						<div class="widget">
							...
							<div class="widget-body bordered-left bordered-warning">
									...
									<select id="parent-id" name="cate_id" style="width:100%;">
										@if(isset($cateList) && $cateList)
											@foreach($cateList as $one)
												<option value="{{$one['id']}}" {{(isset($pid) && $pid == $one['id']) ? "selected" : ''}}>{{$one['name']}}</option>
												@if(isset($one['child']) && $one['child'])
													@foreach($one['child'] as $two)
														<option value="{{$two['id']}}" {{(isset($pid) && $pid == $two['id']) ? "selected" : ''}}>&emsp;&emsp;{{$two['name']}}</option>
														@if(isset($two['child']) && $two['child'])
															@foreach($two['child'] as $three)
																<option value="{{$three['id']}}" {{(isset($pid) && $pid == $three['id']) ? "selected" : ''}}>&emsp;&emsp;&emsp;&emsp;{{$three['name']}}</option>
															@endforeach
														@endif
													@endforeach
												@endif
								...
								<div class="form-group">
									...
									<input type="text" name="title"  class="form-control" id="exampleInputEmail2" placeholder="标题">
								</div>
								<div class="form-group">
									<label class="sr-only" for="exampleInputPassword2">Password</label>
									<input type="text" name="author" class="form-control" id="exampleInputPassword2" placeholder="作者">
								</div>
		...
		<div class="row">
			<div class="col-lg-12 col-sm-12 col-xs-12">
				<div id="editor" name="content" type="text/plain" style="width:100%;height:500px;"></div>
			</div>
		</div>
	</form>
@endsection
