				...
				<div class="widget-body" style="box-shadow:none">
					<div>
						<form class="form-horizontal form-bordered" id="dataForm" data-role="form">
							{{ csrf_field() }}
							<input type="hidden" class="form-control" name="id" value="{{$menu['id']}}" id="" placeholder="">
									...
									<select id="parent-id" name="parent_id" style="width:100%;">
										...
										@if(isset($menuList) && $menuList)
											@foreach($menuList as $one)
												<option value="{{$one['id']}}" {{$menu['parent_id'] == $one['id'] ? "selected" : ""}}>&emsp;&emsp;|___{{$one['name']}}</option>
												@if(isset($one['child']) && $one['child'])
													@foreach($one['child'] as $two)
														<option value="{{$two['id']}}" {{$menu['parent_id'] == $two['id'] ? "selected" : ""}}>&emsp;&emsp;&emsp;&emsp;|___{{$two['name']}}</option>
														@if(isset($two['child']) && $two['child'])
															@foreach($two['child'] as $three)
																<option value="{{$three['id']}}" {{$menu['parent_id'] == $three['id'] ? "selected" : ""}}>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;|___{{$three['name']}}</option>
															@endforeach
														@endif
													@endforeach
												@endif
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<input type="text" class="form-control" name="name" value="{{$menu['name']}}" id="" placeholder="">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<input type="text" class="form-control" name="url" value="{{$menu['url']}}" id="" placeholder="">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<input type="text" class="form-control" name="icon" value="{{$menu['icon']}}" id="" placeholder="">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<input type="text" class="form-control" name="sort" value="{{$menu['sort']}}" id="" placeholder="">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
									<div class="radio">
										<label>
											<input name="display" type="radio" {{$menu['display'] == 1 ? 'checked' : ''}} value="1">
											<span class="text">显示 </span>
										</label>
										<label>
											<input name="display" type="radio" value="2" {{$menu['display'] == 2 ? 'checked' : ''}} class="inverted">
